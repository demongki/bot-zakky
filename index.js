...
if (isLimit(sender)) return reply(dpa.limitend)
reply(dpa.wait)
				dapuhy = await getBuffer(`https://api.lolhuman.xyz/api/random/cecan?apikey=${LolKey}`)
...
if (isLimit(sender)) return reply(dpa.limitend)
reply(dpa.wait)
				dapuhy = await getBuffer(`https://api.lolhuman.xyz/api/random/cogan?apikey=${LolKey}`)
				dp.sendMessage(from, dapuhy, image, {quoted: freply})
				await limitAdd(sender)
				break
...
//ANIME MENU
                ...
                dp.sendMessage(from, wibutolol, video, {quoted: dap})
                break
                case 'wait':
				if (!isUser) return reply(dpa.noregis)
				if (isBanned) return reply(dpa.wait)
